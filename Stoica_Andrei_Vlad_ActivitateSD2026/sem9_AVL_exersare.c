...
void rotireStanga(NodA** root)
{
	NodA* temp = (*root)->right;
	(*root)->right = temp->left;
	temp->left = (*root);
	(*root) = temp;
}

void rotireDreapta(NodA** root)
{
	NodA* temp = (*root)->left;
...
void adaugaMasinaInArboreEchilibrat(NodA** root, Masina masinaNoua) {
	if ((*root) == NULL)
	{
...
void dezalocareArboreDeMasini(NodA** root) {
	if ((*root))
	{
...
int main() {
	...
	afisareMasina(m1);
	printf("-----------------------------------------\n");
	NodA* root = citireArboreDeMasiniDinFisier("masini.txt");
	...
	float pretulMasinilorUnuiSofer = calculeazaPretulMasinilorUnuiSofer(root, "Gigel");
